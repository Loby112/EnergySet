...
namespace TCPEnergy{
    public class Program{
        ...
        public static void HandleClient(TcpClient socket){
                ...
                energy = JsonSerializer.Deserialize<EnergyData>(message);
                if (energy.EnergyMetric == "Calorie"){
                    double result = EnergyConverter.ToJoule(energy.Value);
                    writer.WriteLine("Result is " + result);
                }
                if (energy.EnergyMetric == "Joule") {
                    double result = EnergyConverter.ToCalorie(energy.Value);
                    writer.WriteLine("Result is " + result);
                }
